...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (showHistory) {
      const loadHistory = async () => {
        try {
          const response = await axios.get("http://localhost:5173/storage");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5173/diagnose", {
  ...
  const clearHistory = async () => {
    try {
    await axios.delete("http://localhost:5173/clear-history");
    setHistory([]); 
    setErrorMessage("");
    } catch (error) {
