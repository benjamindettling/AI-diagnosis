...
const defaultsymptomsList = ["Itching", "Skin Rash", "Nodal Skin Eruptions", "Continuous Sneezing", "Shivering",
    "Chills", "Joint Pain", "Stomach Pain",
    "Acidity", "Ulcers on Tongue", "Muscle Wasting", "Vomiting",
    "Burning Micturition", "Fatigue", "Weight Gain", "Anxiety", "Cold Hands and Feet",
    ...
    "Abdominal Pain", "Diarrhoea", "Mild Fever", "Yellow Urine", "Yellowing of Eyes", "Acute Liver Failure",
    ...
    "Chest Pain", "Weakness in Limbs", "Fast Heart Rate", "Pain During Bowel Movements", "Pain in Anal Region",
    ...
    "Palpitations", "Painful Walking", "Pus-Filled Pimples", "Blackheads", "Scarring", "Skin Peeling",
    "Silver-Like Dusting", "Small Dents in Nails",
    "Inflammatory Nails", "Blister", "Red Sore Around Nose", "Yellow Crust Ooze", "Hypothermia"];


// Fetch symptoms dynamically from the backend
// const fetchSymptoms = async (searchTerm: string): Promise<string[]> => {
//   try {
//     const response = await axios.get("http://localhost:5173/query", {
...
const DDL: React.FC<DDLProps> = ({ selectedSymptoms, onSymptomSelect }) => {
  ...
  const [symptomsList, setSymptomsList] = useState<string[]>([...defaultsymptomsList]);

  const fetchSymptomsFromGPT = async (input: string) => {
    try {
      const response = await axios.post("http://localhost:5173/query", {
        query: input,
      });
  ...
  return (
      <div className="dropdown-container">
          <input
              ...
              onKeyDown={(e) => (
                  e.key === 'Enter' ? toggleListVisibility() : null
              )}
          />

          <button onClick={toggleListVisibility}>
              {isListVisible ? <i className="fa-solid fa-caret-up"></i> : <i className="fa-solid fa-caret-down"></i>}
          </button>
